...
{
	if (x - 1 >= 0 && y - 1 >= 0 && board[y - 1][x - 1] == 'K')
		return 1;
	if (x + 1 < size && y - 1 >= 0 && board[y - 1][x + 1] == 'K')
		return 1;
	return 0;
}
...
int	checkmate(int size, char *board[])
{
	...
	while (row < size)
	{
		...
		while (column < size)
		{
			...
			{
				if (pawn(row, column, size, board))
					return 1;
			} else if (board[column][row] == 'B')
			{
				if (bishop(row, column, size, board))
					return 1;
			} else if (board[column][row] == 'R')
			{
				if (rook(row, column, size, board))
					return 1;
			} else if (board[column][row] == 'Q')
			{
				if (bishop(row, column, size, board) || rook(row, column, size, board))
					return 1;
			}
			column++;
		}
